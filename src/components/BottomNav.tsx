    ...
    <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-50" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
      <div className="flex justify-around items-center h-16 max-w-lg mx-auto">
        {tabs.map(t => {
          ...
          return (
            <button key={t.path} onClick={() => nav(t.path)} className="flex flex-col items-center gap-0.5 min-w-[60px]">
              <t.icon size={22} className={active ? 'text-[#FF6B35]' : 'text-gray-400'} />
