...
export default function Estimate() {
  ...
  const shareWhatsApp = () => {
    const text = `Moving Estimate:\n📦 ${estimate.total_volume} cuft (${cuftToM3(estimate.total_volume)} m³)\n⚖️ ${estimate.total_weight} kg\n🚛 ${estimate.recommended_vehicle}\n💰 ${formatCurrency(costs.low.total, currency)} - ${formatCurrency(costs.high.total, currency)}\n\nGenerated by Moving Estimator`
                        ...
                        {room.items.map(item => (
                          <div key={item.id} className="flex items-center gap-2 bg-gray-50 rounded-lg p-2.5 text-xs">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-[10px] text-gray-400">{item.length_cm}×{item.width_cm}×{item.height_cm}cm · {item.weight_kg}kg</p>
                            </div>
            ...
            <div className="flex items-end justify-between mb-4">
              <div>
                ...
                <p className="text-2xl font-bold text-[#1E3A5F]">
                  {formatCurrency(costs.low.total, currency)} <span className="text-sm font-normal text-gray-400">–</span> {formatCurrency(costs.high.total, currency)}
                </p>
              </div>
